...
${circleRR[@]})
input=(${_IN[@]} ${_iN[@]})
lookAhead=(${outcgravitySmallRN[@]} ${gravityCapytalMN[@]})
...
# ○右が Vの小文字、V で その右が左寄り、右寄り、均等、中間の文字の場合 s 右に移動 (次の処理とセット)
backtrack1=("")
backtrack=("")
input=(${_sN[@]})
...
# △左が中間の小文字で 右が右寄り、均等、丸い文字の場合 L 以外の左寄りの文字 元に戻る
