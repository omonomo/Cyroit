...
　今のところ、run_ff_ttx.sh はオプションの複数指定に対応していません。  
　カスタマイズする場合は run_ff_ttx.sh を直接編集するか、font_generator.sh を実行後 table_modificator.sh を実行してください。  
...
　またオプションの後に素材にするフォントファイル名を指定する必要がありますが、基本的に auto で問題ありません。
...
`-o` オブリーク体を生成しません。  
`-d` 下書きモードで生成します。時間のかかる処理を飛ばしますが、完成品と見た目が少し異なる文字があります。  
　　改変グリフ確認用です。  
`-S` ss 対応のフォントを生成します。自動的に -Zzts オプション が ON、-bc オプションが OFF になります。  
...
　フォント合成時に失われた、IVS 関連の cmap テーブルを作成し直します。  
　作成したデータは table_modificator.sh で使用します。  
...
　　先に uvs_table_maker.sh を実行し、`gsubList.txt` が生成されていれば省略しても問題ありません。  
